...
void S2EExecutor::stateSwitchTimerCallback(void *opaque) {
    ...
    libcpu_mod_timer(c->m_stateSwitchTimer, libcpu_get_clock_ms(host_clock) + 1000);
}
...
S2EExecutor::StatePair S2EExecutor::fork(ExecutionState &current, const klee::ref<Expr> &condition,
    ...
    for (unsigned i = 0; i < 2; ++i) {
        if (VerboseFork) {
            ...
            out << "    state " << newStates[i]->getID() << "\n";
			newStates[i]->getStaticBranchTargets(&staticTargets[0], &staticTargets[1]);
			out << "Target Branch[" << i << "]:"
			<< " state: " << newStates[i]->getID() << " dest:" << hexval(staticTargets[i]) << "\n";
        }
