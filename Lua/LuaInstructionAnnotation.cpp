...
void LuaInstructionAnnotation::onTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state,
    ...
    //getDebugStream(state) << "[TESTLOG] onTranslateBlockStart0 " << "\n";

    auto module = m_modules->getModule(state, pc);
    if (!module) {
        return;
    }
    //getDebugStream(state) << "[TESTLOG] onTranslateBlockStart1 " << "\n";
    ...
    //getDebugStream(state) << "[TESTLOG] onTranslateBlockStart2 " << "\n";
...
void LuaInstructionAnnotation::onTranslateInstructionStart(ExecutionSignal *signal, S2EExecutionState *state,
    ...
    //getDebugStream(state) << "[TESTLOG] onTranslateInstructionStart1 " << "\n";

    if (annotations->find(tofind) == annotations->end()) {
        return;
    }

    //getDebugStream(state) << "[TESTLOG] onTranslateInstructionStart2 " << "\n";
...
void LuaInstructionAnnotation::onInstruction(S2EExecutionState *state, uint64_t pc,
    ...
    //getDebugStream(state) << "[TESTLOG] instruction address1 " << hexval(modulePc) << "\n";
    
    // if( (modulePc & 0xffff) == (pc & 0xffff) ){
    //     getDebugStream(state) << "[TESTLOG] instruction address2 " << hexval(modulePc) << "\n";
    // }
